#!/usr/bin/bash
#SBATCH --job-name=ichor_pipeline_full_100kb
#SBATCH --nodes=1
...
mkdir -p $in/results/ichor -p $in/results/ichor/100k
...
#2. Ichor Prep
in_util=/${patient}
out_util=/${patient}/results/ichor/100k

#3. Run Ichor
in_ichor=/${patient}/results/ichor/100k
out_ichor=/${patient}/results/ichor/100k
...
#2. Ichor Prep
$hmm_util/readCounter --window 100000 \
--quality 20 \
--chromosome "chr1,chr2,chr3,chr4,chr5,chr6,chr7,chr8,chr9,chr10,chr11,chr12,chr13,chr14,chr15,chr16,chr17,chr18,chr19,chr20,chr21,chr22,chrX,chrY" $in_util/$file_name > $out_util/${patient}_100kb.wig

#3. Run Ichor
Rscript $ichor/scripts/runIchorCNA.R --id $patient \
  --WIG $in_ichor/${patient}_100kb.wig --ploidy "c(2)" --normal "c(0.95, 0.99, 0.995, 0.999)" --maxCN 5 \
  --gcWig $ichor/inst/extdata/gc_hg38_100kb.wig \
  --mapWig $ichor/inst/extdata/map_hg38_100kb.wig \
