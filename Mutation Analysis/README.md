**Mutation Analysis:** Can we identify tumour associated mutations in the five patient samples.

**Files -**

**/Mutation Analysis Steps:** Full pipeline broken up into individual steps  to run as needed.

**Mutation_Analysis_Full.sh:** Full pipeline including intermediate file cleanup

Caution: All code has been sanitized prior to uploading. Therefore, do not expect any code to run _as is_. You will need to alter directory pathways and file names where necessary. The pipelines have been designed in a way that places all of this information near the top, making it as easy as possible to get up and running.


**References:**

Agilent Technologies Inc (2024) Avida DNA targeted sequencing analysis: Technical guide. Available at: https://www.agilent.com/cs/library/usermanuals/public/G9409-90001.pdf (Accessed: 28 July 2026).

Bonfield, J.K. et al. (2021) 'HTSlib: C library for reading/writing high-throughput sequencing data', GigaScience, 10(2), pp. giab007. doi: 10.1093/gigascience/giab007. Available at: https://doi.org/10.1093/gigascience/giab007 .

Broad Institute. (2019) Picard toolkit Available at: https://github.com/broadinstitute/picard (Downloaded: 17 July 2026).

Fulcrum Genomics. (2022) Fgbio Available at: https://github.com/fulcrumgenomics/fgbio (Downloaded: 18 July 2026).

Lai, Z. et al. (2016) 'VarDict: A novel and versatile variant caller for next-generation sequencing in cancer research', Nucleic Acids Research, 44(11), pp. e108. Available at: https://doi.org/10.1093/nar/gkw227 .

Li, H. (2013) Aligning sequence reads, clone sequences and assembly contigs with BWA-MEM Available at: https://arxiv.org/abs/1303.3997 (Downloaded: 17 July 2026).

Quinlan, A.R. and Hall, I.M. (2010) 'BEDTools: A flexible suite of utilities for comparing genomic features', Bioinformatics (Oxford, England), 26(6), pp. 841–842. Available at: https://doi.org/10.1093/bioinformatics/btq033 .

R Core Team. (2023) R: A language and environment for statistical computing Available at: https://www.R-project.org/ (Downloaded: 28 May 2026).


This research used the ALICE High Performance Computing facility at the University of Leicester.
